...
face_cascade = cv2.CascadeClassifier('lib/lbpcascade_frontalface_improved.xml')
...
if os.path.exists(model_path) and os.path.exists(label_path):
    ...
    input_type = '' # Set this to 'c' if you want to use your webcam
    ...
    while True:
        ret, frame = cap.read()
        if input_type == '':
            frame = cv2.imread('old_code/known_faces/Ryujin/123 Ryujin.jpg') # change this after desired image
        ...
        for face in faces:
            ...
            if conf > 0.9:
                ...
                cv2.putText(frame, str(round(prob*100)) + '%', (x, y+h+15), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
                print(f'AI Confidence [{label} - {prob*100:.2f}%]')
            eyes = eyes_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=40)
            for (ex, ey, ew, eh) in eyes:
                cv2.rectangle(frame, (ex, ey), (ex + ew, ey + eh), (255, 255, 0), 2)
            nose = nose_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=50)
    ...
    for person in os.listdir(data_path):
        ...
        for image_path in image_paths:
            ...
            if len(faces) == 0:
                ...
                print(f"{len(faces)} face(s) found in {image_path}")
    ...
    history = model.fit(datagen.flow(X_train.reshape(-1, 64, 64, 1), y_train, batch_size=32), epochs=70, validation_data=(X_test, y_test), verbose=1, callbacks=[lr_scheduler])
